...
export function ProBadge({ className = '' }: { className?: string }) {
  return (
    <span className={`inline-flex items-center gap-0.5 rounded-full bg-amber-100 px-1.5 py-0.5 text-[9px] font-bold uppercase tracking-wider text-amber-700 ${className}`}>
      <Crown size={8} /> PRO
    </span>
  );
}
  ...
  return (
    <>
      <button
        ...
        className={`flex items-center gap-1.5 text-[#9ca3af] ${className}`}
      >
        <Lock size={12} />
      ...
      {showModal && (
        <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-black/50 px-4 pb-4" onClick={() => setShowModal(false)}>
          <div className="bg-white rounded-2xl p-6 max-w-sm w-full shadow-xl" onClick={e => e.stopPropagation()}>
            ...
            <button
              onClick={() => setShowModal(false)}
              className="block w-full text-center mt-2 py-2 text-xs text-[#9ca3af]"
            >
              Maybe later
            </button>
          </div>
