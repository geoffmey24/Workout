      ...
      {parts.map((part, i) => {
        ...
        return (
          <div key={i}>
            <ReactMarkdown
              components={{
                p: ({ children }) => <p className="mb-3 last:mb-0 leading-relaxed text-[var(--text-secondary)]">{children}</p>,
                ol: ({ children }) => <ol className="mb-3 pl-5 list-decimal space-y-1">{children}</ol>,
                ul: ({ children }) => <ul className="mb-3 pl-5 list-disc space-y-1 text-[var(--text-secondary)]">{children}</ul>,
                li: ({ children }) => <li className="leading-relaxed">{children}</li>,
                h1: ({ children }) => <h1 className="font-extrabold text-lg mt-6 mb-3 text-[var(--text-primary)] border-b border-[var(--border)] pb-2">{children}</h1>,
                h2: ({ children }) => <h2 className="font-bold text-base mt-5 mb-2 text-[var(--text-primary)]">{children}</h2>,
                ...
                td: ({ children }) => <td className="px-4 py-3 text-[var(--text-secondary)] whitespace-nowrap">{children}</td>,
                code: ({ children, className }) => {
                  ...
                  return <code className="bg-[var(--bg-elevated)] px-1.5 py-0.5 rounded text-xs font-mono">{children}</code>;
                },
