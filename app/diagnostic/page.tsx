...
import { ArrowLeft, ArrowRight, Check, Dumbbell, Timer, X, ChevronRight, Scale } from 'lucide-react';
import {
    ...
    return (
      <div className="min-h-screen bg-[#f8f9fa]">
        <div className="flex items-center gap-3 border-b border-[#e5e7eb] bg-white px-4 py-3">
          <button onClick={() => router.push('/')} className="text-[#9ca3af] hover:text-[#111827]"><ArrowLeft size={20} /></button>
          <h1 className="font-bold text-sm text-[#111827]">Strength Assessment</h1>
        </div>
        <div className="px-4 py-6">
          <CoachBubble message="Bodyweight programs don't need weight prescriptions. You're all set!" />
          <button onClick={() => router.push('/')}
            className="w-full rounded-xl bg-[#1e3a5f] py-3.5 text-sm font-semibold text-white hover:bg-[#162d4a] transition-colors flex items-center justify-center gap-2">
  ...
  return (
    <div className="min-h-screen bg-[#f8f9fa]">
      ...
      <div className="flex items-center gap-3 border-b border-[#e5e7eb] bg-white px-4 py-3">
        ...
        <h1 className="font-bold text-sm text-[#111827] flex-1">Strength Assessment</h1>
      ...
      <div className="h-1 bg-[#e5e7eb]">
        <div className="h-full bg-[#1e3a5f] transition-all duration-300 ease-out" style={{ width: `${getProgress()}%` }} />
      </div>
        ...
        <div key={slideKey} className={slideDir === 'forward' ? 'slide-forward' : 'slide-backward'}>
              ...
              <div className="space-y-6">
                <div className="text-center py-4">
                  <input
                    ...
                    placeholder="180"
                    className="text-6xl font-bold text-[#1e3a5f] text-center w-full bg-transparent border-none outline-none placeholder-[#1e3a5f]/20"
                    autoFocus
                  />
                  <p className="text-sm text-[#9ca3af] mt-2">{unit}</p>
                </div>
                <div className="flex gap-2 justify-center">
                  <button onClick={() => setUnit('lbs')}
                    className={`rounded-xl px-6 py-2.5 text-sm font-semibold transition-colors ${unit === 'lbs' ? 'bg-[#1e3a5f] text-white' : 'bg-white border border-[#e5e7eb] text-[#9ca3af]'}`}>
                    lbs
                  </button>
                  <button onClick={() => setUnit('kg')}
                    className={`rounded-xl px-6 py-2.5 text-sm font-semibold transition-colors ${unit === 'kg' ? 'bg-[#1e3a5f] text-white' : 'bg-white border border-[#e5e7eb] text-[#9ca3af]'}`}>
                    kg
                  </button>
                </div>
                <button onClick={() => { if (bodyWeight && parseFloat(bodyWeight) > 0) goForward('exercises'); }}
                  disabled={!bodyWeight || parseFloat(bodyWeight) <= 0}
                  className="w-full flex items-center justify-center gap-2 rounded-xl bg-[#1e3a5f] py-3.5 text-sm font-semibold text-white hover:bg-[#162d4a] transition-colors disabled:opacity-40">
            ...
            <>
              ...
              <div className="space-y-3 mb-6">
                {allExercises.map(ex => {
                  ...
                  return (
                    <div key={ex}
                      className={`flex items-center justify-between rounded-xl border p-4 min-h-[56px] transition-all ${isSelected ? 'border-[#1e3a5f] bg-[#1e3a5f]/10' : 'border-[#e5e7eb] bg-white opacity-50'}`}>
                      <div className="flex items-center gap-3">
                        <Dumbbell size={20} className="text-[#1e3a5f]" />
                        <span className="text-sm font-semibold text-[#111827]">{ex}</span>
                      </div>
                      <button onClick={() => {
                        ...
                        className={`w-8 h-8 rounded-full flex items-center justify-center transition-colors ${isSelected ? 'bg-[#1e3a5f]/10 text-[#1e3a5f] hover:bg-red-50 hover:text-red-500' : 'bg-[#e5e7eb] text-[#9ca3af]'}`}>
              ...
              <p className="text-xs text-[#9ca3af] mb-4 text-center">Tap X to remove exercises you can&apos;t do. Minimum 2 required.</p>
              <button onClick={() => {
                setCurrentExIdx(0);
                setSet1Weight(String(defaultStart));
                goForward('warmup1');
                ...
                className="w-full flex items-center justify-center gap-2 rounded-xl bg-[#1e3a5f] py-3.5 text-sm font-semibold text-white hover:bg-[#162d4a] transition-colors disabled:opacity-40">
                Start Testing ({selectedExercises.length} exercises) <ArrowRight size={16} />
              </button>
            </>
          )}
          ...
          {phase === 'warmup1' && (
            <>
              ...
              <div className="rounded-2xl bg-white border border-[#e5e7eb] p-5">
                <p className="text-xs font-medium text-[#9ca3af] uppercase tracking-wider mb-1">Exercise {currentExIdx + 1} of {totalExercises}</p>
                <h2 className="text-xl font-bold text-[#1e3a5f] mb-6">{selectedExercises[currentExIdx]}</h2>
                <div className="mb-6">
                  <label className="block text-xs font-medium text-[#9ca3af] mb-2">Weight used ({unit})</label>
                  <input type="number" inputMode="decimal" value={set1Weight}
                    onChange={e => setSet1Weight(e.target.value)}
                    className="w-full rounded-xl border border-[#e5e7eb] bg-white px-4 py-4 text-2xl font-bold text-center text-[#1e3a5f] focus:border-[#1e3a5f] focus:outline-none focus:ring-1 focus:ring-[#1e3a5f]"
                    autoFocus />
                </div>
                <button onClick={handleSet1Done}
                  disabled={!set1Weight || parseFloat(set1Weight) <= 0}
                  className="w-full flex items-center justify-center gap-2 rounded-xl bg-[#1e3a5f] py-3.5 text-sm font-semibold text-white hover:bg-[#162d4a] transition-colors disabled:opacity-40">
                  Done — Next Set <ArrowRight size={16} />
                </button>
              </div>
            </>
          )}
          ...
          {phase === 'warmup2' && (
            <>
              ...
              <div className="rounded-2xl bg-white border border-[#e5e7eb] p-5">
                <p className="text-xs font-medium text-[#9ca3af] uppercase tracking-wider mb-1">Exercise {currentExIdx + 1} of {totalExercises}</p>
                <h2 className="text-xl font-bold text-[#1e3a5f] mb-2">{selectedExercises[currentExIdx]}</h2>
                <p className="text-xs text-[#6b7280] mb-6">Suggested: {set2Weight} {unit}</p>
                <div className="mb-6">
                  <label className="block text-xs font-medium text-[#9ca3af] mb-2">Weight used ({unit})</label>
          ...
          {phase === 'warmup3' && (
            <>
              ...
              <div className="rounded-2xl bg-white border border-[#e5e7eb] p-5">
                <p className="text-xs font-medium text-[#9ca3af] uppercase tracking-wider mb-1">Exercise {currentExIdx + 1} of {totalExercises}</p>
                <h2 className="text-xl font-bold text-[#1e3a5f] mb-2">{selectedExercises[currentExIdx]}</h2>
                <p className="text-xs text-[#6b7280] mb-6">Suggested: {set3Weight} {unit}</p>
                <div className="mb-6">
                  <label className="block text-xs font-medium text-[#9ca3af] mb-2">Weight used ({unit})</label>
          ...
          {phase === 'working' && (
            <>
              ...
              <div className="rounded-2xl bg-white border border-[#e5e7eb] p-5">
                <p className="text-xs font-medium text-[#9ca3af] uppercase tracking-wider mb-1">Exercise {currentExIdx + 1} of {totalExercises}</p>
                <h2 className="text-xl font-bold text-[#1e3a5f] mb-2">{selectedExercises[currentExIdx]}</h2>
                <p className="text-xs text-[#6b7280] mb-6">Suggested: {set4Weight} {unit}</p>
                <div className="space-y-4 mb-6">
                  ...
                  <div>
                    <label className="block text-xs font-medium text-[#9ca3af] mb-2">Reps completed</label>
                    <input type="number" inputMode="numeric" value={workingReps}
                      onChange={e => setWorkingReps(e.target.value)}
                      placeholder="8"
                      className="w-full rounded-xl border border-[#e5e7eb] bg-white px-4 py-4 text-2xl font-bold text-center text-[#1e3a5f] focus:border-[#1e3a5f] focus:outline-none focus:ring-1 focus:ring-[#1e3a5f] placeholder-[#1e3a5f]/20" />
                  </div>
                </div>
                {set4Weight && workingReps && parseFloat(set4Weight) > 0 && parseInt(workingReps) > 0 && (
                  <p className="text-xs text-[#1e3a5f] font-medium mb-4 text-center">
                ...
                <button onClick={handleWorkingDone}
                  ...
                  className="w-full flex items-center justify-center gap-2 rounded-xl bg-[#1e3a5f] py-3.5 text-sm font-semibold text-white hover:bg-[#162d4a] transition-colors disabled:opacity-40">
                  Rate Difficulty <ArrowRight size={16} />
                </button>
              </div>
            </>
          )}
            ...
            <>
              ...
              {phase === 'rpe_retry' && (
                <div className="rounded-2xl bg-white border border-[#e5e7eb] p-5 mb-4">
                  <p className="text-xs font-medium text-[#9ca3af] uppercase tracking-wider mb-1">{selectedExercises[currentExIdx]} — Retry</p>
                  <div className="space-y-4 mb-4">
                    <div>
                      <label className="block text-xs font-medium text-[#9ca3af] mb-2">Weight ({unit})</label>
                      <input type="number" inputMode="decimal" value={set4Weight}
                        onChange={e => setSet4Weight(e.target.value)}
                        className="w-full rounded-xl border border-[#e5e7eb] bg-white px-4 py-4 text-2xl font-bold text-center text-[#1e3a5f] focus:border-[#1e3a5f] focus:outline-none focus:ring-1 focus:ring-[#1e3a5f]" />
                    </div>
              ...
              {rpeMessage && selectedRpe !== null && (
                <div className="rounded-xl bg-[#eef2ff] border border-[#1e3a5f]/20 px-4 py-3 mb-4">
                  <p className="text-sm text-[#1e3a5f] font-medium">{rpeMessage}</p>
                </div>
              )}
              ...
              <div className="flex flex-col items-center py-8">
                <div className="relative w-48 h-48 mb-6">
                  ...
                  <div className="absolute inset-0 flex flex-col items-center justify-center">
                    ...
                    <span className="text-xs text-[#9ca3af] mt-1">remaining</span>
                  </div>
                </div>
                <p className="text-sm text-[#6b7280] mb-4">
                  Next: <span className="font-semibold text-[#111827]">{selectedExercises[currentExIdx]}</span>
                </p>
                <button onClick={skipRest}
                  className="rounded-xl border border-[#e5e7eb] bg-white px-6 py-2.5 text-sm font-medium text-[#9ca3af] hover:text-[#111827] hover:border-[#1e3a5f]/40 transition-colors">
                  Skip Rest
                </button>
              </div>
            </>
            ...
            <>
              ...
              <div className="rounded-2xl bg-white border border-[#e5e7eb] p-5 shadow-sm">
                <div className="text-center mb-5">
                  <Scale size={32} className="mx-auto text-[#1e3a5f] mb-2" />
                  <h2 className="text-lg font-bold text-[#111827]">Your Strength Profile</h2>
                  <p className="text-sm text-[#6b7280] mt-1">Body weight: {bodyWeight} {unit}</p>
                </div>

                <div className="space-y-4 mb-6">
                  {results.map((r, i) => (
                    <div key={i} className="rounded-xl bg-[#f8f9fa] border border-[#e5e7eb] p-4">
                      <div className="flex justify-between items-start mb-2">
                        <h3 className="font-semibold text-sm text-[#111827]">{r.exercise}</h3>
                      ...
                      <div className="flex justify-between mt-1">
                        {LEVEL_ORDER.map(level => (
                          <span key={level} className={`text-[8px] ${level === r.level ? 'text-[#111827] font-bold' : 'text-[#9ca3af]'}`}>
                            {level.slice(0, 3)}
                          </span>
                        ))}
                      </div>
                ...
                {/* Overall */}
                <div className="text-center mb-6 py-3 rounded-xl bg-[#1e3a5f]/5">
                  <p className="text-xs text-[#9ca3af] uppercase tracking-wider">Overall Level</p>
                  <p className="text-lg font-bold text-[#1e3a5f]">{getOverallLevel(results)}</p>
                </div>
                ...
                <button onClick={handleSave}
                  ...
                  <Check size={18} /> Save and Start Training
                </button>
              </div>
            </>
          )}
          ...
          <div className="rounded-2xl bg-white border border-[#e5e7eb] p-4">
            ...
            {results.map((r, i) => (
              <div key={i} className="flex items-center justify-between py-1.5 text-sm">
                ...
                <span className="text-[#6b7280]">{r.workingWeight} {unit} (1RM: {r.adjusted1RM})</span>
              </div>
            ))}
          </div>
        </div>
