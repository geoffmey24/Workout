    ...
    return (
      <div className="min-h-screen pb-24">
        ...
        <div className="flex items-center justify-center py-20">
          <div className="flex gap-1.5"><span className="typing-dot h-2 w-2 rounded-full bg-[#1e3a5f]/100" /><span className="typing-dot h-2 w-2 rounded-full bg-[#1e3a5f]/100" /><span className="typing-dot h-2 w-2 rounded-full bg-[#1e3a5f]/100" /></div>
        </div>
        <Navigation />
      </div>
    );
  ...
  return (
    <div className="min-h-screen pb-24">
      <div className="flex items-center gap-3 border-b border-[#e5e7eb] bg-white px-4 py-3">
        ...
        <button onClick={() => { setCompleted({}); setWorkoutDone(false); setExerciseWeights({}); setWorkoutNote(''); setNoteSaved(false); }} className="text-[#9ca3af] hover:text-[#111827]" title="Reset">
          <RotateCcw size={16} />
        </button>
      </div>
      ...
      {totalWorkouts > 0 && (
        <div className="px-4 py-3 flex gap-3">
          <div className="flex-1 rounded-xl bg-white border border-[#e5e7eb] p-3 text-center">
            ...
            <p className="text-lg font-bold text-[#111827]">{streak}<span className="text-xs text-[#9ca3af] ml-0.5">d</span></p>
          </div>
          <div className="flex-1 rounded-xl bg-white border border-[#e5e7eb] p-3 text-center">
      ...
      <div className="px-4 mb-3 flex gap-2">
        <button onClick={() => setViewMode('checklist')} className={`flex-1 rounded-xl py-2 text-xs font-semibold transition-colors ${viewMode === 'checklist' ? 'bg-[#1e3a5f] text-white' : 'bg-white border border-[#e5e7eb] text-[#9ca3af]'}`}>
          Checklist ({completedCount}/{totalExercises})
        </button>
        <button onClick={() => setViewMode('prs')} className={`flex-1 rounded-xl py-2 text-xs font-semibold transition-colors ${viewMode === 'prs' ? 'bg-[#1e3a5f] text-white' : 'bg-white border border-[#e5e7eb] text-[#9ca3af]'}`}>
          PRs
        </button>
        <button onClick={() => setViewMode('full')} className={`flex-1 rounded-xl py-2 text-xs font-semibold transition-colors ${viewMode === 'full' ? 'bg-[#1e3a5f] text-white' : 'bg-white border border-[#e5e7eb] text-[#9ca3af]'}`}>
          Full Program
        </button>
      </div>
      ...
      <div className="px-4 mb-4">
        <div className="h-2 rounded-full bg-[#e5e7eb]">
          <div className={`h-full rounded-full transition-all duration-300 ${progress === 100 ? 'bg-[#4ade80]/100' : 'bg-[#1e3a5f]'}`} style={{ width: `${progress}%` }} />
        </div>
      </div>
      ...
      {viewMode === 'checklist' ? (
        <div className="px-4 space-y-2">
          {exercises.length > 0 ? exercises.map((ex, i) => {
            ...
            return (
              <div key={key} className={`rounded-xl border p-3 transition-all ${done ? 'border-[#4ade80]/30 bg-[#4ade80]/10' : 'border-[#e5e7eb] bg-white'}`}>
                <button onClick={() => toggleExercise(key, i)} className="w-full flex items-center gap-3 text-left">
                  ...
                  <p className={`text-sm flex-1 ${done ? 'line-through text-[#6b7280]' : 'text-[#111827]'}`}>{ex}</p>
                </button>
                {lastEntry && !done && (
          ...
          {/* Workout Note */}
          <div className="mt-4 rounded-xl bg-white border border-[#e5e7eb] p-3">
        ...
        <div className="px-4 space-y-2">
          ...
          {prEntries.length === 0 ? (
            ...
            return (
              <div key={name} className="rounded-xl bg-white border border-[#e5e7eb] p-3 flex items-center justify-between">
                <div>
      ...
      <div className="px-4 mt-4">
        <Link href="/chat?topic=I%20need%20alternative%20exercises%20for%20my%20current%20workout.%20What%20can%20I%20swap%20in%3F" className="flex items-center gap-2 rounded-xl border border-[#e5e7eb] bg-white px-4 py-3 hover:border-[#1e3a5f]/50 transition-colors">
